...
public class MediaCenterView extends javax.swing.JFrame implements DSSObserver {
    ...
    private void PlayMusicButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_PlayMusicButtonActionPerformed
        ...
        try {
            ...
            if (this.getExtensionOfFile(list_content_selected).equals("mp4")) {
            
                System.out.println("Playing video");
                
                String video_path = "DB/Content/Music/" + list_content_selected;

                System.out.println("Preparing to play: " + video_path);

                Desktop.getDesktop().open(new File(video_path));
                
                System.out.println("Playing: " + video_path);
            
            } else {
            
                this.currentContentPlayer.end();

                String music_path = "DB/Content/Music/" + list_content_selected;
    ...
    private void updateContentList() {
        ...
        List<String> nomesFicheirosDiretoria = this.controller.listNomesFicheirosDir("DB/Content/Music/");
    
        System.out.println(nomesFicheirosDiretoria.toString());

        DefaultListModel model = new DefaultListModel();
        model.clear();
        
        for(String fich: nomesFicheirosDiretoria) {
            
            model.addElement(fich);
        }
